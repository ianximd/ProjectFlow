...
export default function GraphQLExplorerPage() {
  const sessionToken = useStore((s) => s.accessToken) ?? '';
  ...
  // Token: defaults to the session JWT; can be overridden manually.
  const [overrideToken, setOverrideToken] = useState(false);
  const [tokenOverride, setTokenOverride] = useState('');
  const effectiveToken = overrideToken ? tokenOverride : sessionToken;
  ...
  const run = async () => {
    ...
    try {
      ...
      setLastStatus({ ok: res.ok && !json.errors, statusCode: res.status, ms: elapsed });
  ...
  }, [query, variables, effectiveToken]);
