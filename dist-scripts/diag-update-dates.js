...
async function readDates(req, taskId) {
    const r = await req.query(`SELECT StartDate, DueDate FROM Tasks WHERE Id = '${taskId}'`);
...
async function main() {
    ...
    const def = await pool.request().query(`SELECT OBJECT_DEFINITION(OBJECT_ID('usp_Task_UpdateDates')) AS Body`);
    const body = def.recordset[0]?.Body ?? null;
    console.log('── Deployed usp_Task_UpdateDates ──────────────────────────────');
    console.log(body ? body.trim() : '!! PROC NOT FOUND IN DATABASE !!');
    console.log('───────────────────────────────────────────────────────────────\n');
    // ── 2. Pick a sample task ────────────────────────────────────────────────
    const sample = await pool.request().query(`SELECT TOP 1 Id, IssueKey, StartDate, DueDate, UpdatedAt
       FROM Tasks
      WHERE DeletedAt IS NULL
      ORDER BY UpdatedAt DESC`);
    const task = sample.recordset[0];
    if (!task) {
    ...
    async function trial(label, bind) {
        const tx = new mssql_1.default.Transaction(pool);
        await tx.begin();
        try {
            const spReq = new mssql_1.default.Request(tx);
            spReq.input('TaskId', mssql_1.default.UniqueIdentifier, task.Id);
            spReq.input('RequesterId', mssql_1.default.UniqueIdentifier, DUMMY_GUID);
            spReq.input('ClearStartDate', mssql_1.default.Bit, 0);
            spReq.input('ClearDueDate', mssql_1.default.Bit, 0);
            bind(spReq); // adds StartDate + DueDate in the shape under test
            await spReq.execute('usp_Task_UpdateDates');
            const after = await readDates(new mssql_1.default.Request(tx), task.Id);
            const changed = fmt(after.StartDate).slice(0, 10) === TEST_START &&
    ...
    await trial('A: sql.Date/DateTime2 + STRING ("2029-09-09")', (req) => {
        req.input('StartDate', mssql_1.default.Date, TEST_START);
        req.input('DueDate', mssql_1.default.DateTime2, TEST_DUE);
    });
    ...
    await trial('B: sql.Date/DateTime2 + JS Date object', (req) => {
        ...
        req.input('StartDate', mssql_1.default.Date, new Date(sy, sm - 1, sd));
        req.input('DueDate', mssql_1.default.DateTime2, new Date(dy, dm - 1, dd));
    });
    // Test C — control: raw inline UPDATE (no proc) to prove the column writes.
    {
        ...
        await tx.begin();
        try {
            await new mssql_1.default.Request(tx).query(`UPDATE Tasks SET StartDate='${TEST_START}', DueDate='${TEST_DUE}' WHERE Id='${task.Id}'`);
            const after = await readDates(new mssql_1.default.Request(tx), task.Id);
            const changed = fmt(after.StartDate).slice(0, 10) === TEST_START &&
                fmt(after.DueDate).slice(0, 10) === TEST_DUE;
