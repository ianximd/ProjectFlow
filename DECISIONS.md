...
## 2026-06-19 — Phase 11a (AI Foundation) — permission-safe retrieval, no LLM answer yet

Stands up the AI layer's foundation: `AiChunks`/`AiRuns` schema (migration `0063`), the `ai.use` permission (migration `0064`), a set-based ACL pre-filter SP, a provider-agnostic gateway with audit, SQL-resident hybrid retrieval with a two-layer permission filter, the BullMQ indexing worker + seams, and an internal `POST /ai/search`. Scope is features 1–5 foundation only — **no LLM answer/citations surface yet (that is 11b)**. Plan `docs/superpowers/plans/2026-06-19-phase11a-ai-foundation.md`; spec `docs/superpowers/specs/2026-06-18-ai-layer-phase11-design.md`. Built subagent-driven (fresh implementer + two-stage spec/quality review per task). Local DB advanced **0062 → 0064**; 393 SP files (+`usp_AccessibleScopes_ForUser`).

### Two-layer permission filter (the headline correctness property)
- **Layer 1 (performance pre-filter):** `usp_AccessibleScopes_ForUser(@UserId,@WorkspaceId)` — a NEW read-only SP that returns the SET of `(ScopeType,ScopeId)` a user can VIEW, ported set-wise from the per-object `usp_ObjectAccess_Resolve` (same floor: owner=FULL / **guest=NULL above member** / member=EDIT; same explicit-grant ancestry via materialized-Path prefix `node.Path LIKE folder.Path + '%'`; same PRIVATE-space exclusion). It is JOINed (inner, on `ScopeType AND ScopeId`) into both candidate queries. Correctness is pinned by an **oracle test** asserting the set SP returns EXACTLY `{nodes where the per-object resolver returns Level IS NOT NULL}` for owner/member/guest/stranger + a ROLE-grant case.
- **Layer 2 (authoritative correctness):** `RetrievalService.retrieve()` does a per-result, awaited `accessService.can(userId, scopeType, scopeId, 'VIEW')` re-check and drops anything denied — the real guarantee. Proven by `retrieval.security.integration.test.ts` **Test 2**, which injects a leaky repo returning ALL chunk ids (SP fully bypassed) and confirms a private-space chunk STILL does not reach a guest. (Adversarially verified: removing the `can()` filter makes Test 2 fail at the leak assertion.) The injected `AccessChecker` type only accepts `(userId, scopeType, scopeId)`, so passing object identity instead of scope identity is a compile error — the classic wrong-argument leak is structurally prevented.
- **Cross-tenant safety:** EVERY `AiChunks` query (getLive / upsert delete+insert / softDelete / keywordCandidates / semanticCandidates / loadChunks) filters by `WorkspaceId`; all read/candidate queries also filter `DeletedAt IS NULL` (soft-deleted chunks are never candidates/loaded/re-checked).

### SQL-resident hybrid retrieval (no pgvector / no ANN)
Keyword + semantic, fused by reciprocal-rank fusion (`reciprocalRankFusion`, k=60), head `k*2`, then Layer-2 filtered to top-k. Embeddings stored as `VARBINARY(MAX)` (Float32Array ↔ little-endian Buffer; read-back copies into an aligned buffer because mssql Buffers aren't 4-byte-aligned). Semantic candidates = **brute-force cosine in JS** over ACL-filtered rows (ponytail O(n) scan; ANN deferred). Re-embedding is skipped when a chunk's `ContentHash` is unchanged (hash governs embedding reuse ONLY, never ACL — a changed ACL re-anchors via the fresh seam scope).

### FTS NOT installed in the local Docker image → keyword degrades to LIKE
`SELECT SERVERPROPERTY('IsFullTextInstalled')` = `0` on `mcr.microsoft.com/mssql/server:2022-latest`. Consequences, all handled portably:
- Migration `0063` guards the `CREATE FULLTEXT CATALOG/INDEX` (and the rollback DROPs) behind `IF CAST(SERVERPROPERTY('IsFullTextInstalled') AS INT)=1` + dynamic SQL (`EXEC(...)`, since `CREATE FULLTEXT` must be first-in-batch). The migration thus creates FTS where available and silently skips where not.
- `keywordCandidates` uses **parameterized `LIKE`** (query split into terms, each `%escaped%` bound as a parameter, LIKE wildcards `% _ [` escaped with `ESCAPE '\'`; user text never concatenated into SQL) instead of `CONTAINS`/`FREETEXT`. Tagged ponytail; **upgrade to `CONTAINS` when an FTS-enabled image is used**. The semantic (FakeEmbedder cosine) path is unaffected.

### Provider-agnostic gateway + audit (no content stored)
`AiGatewayService` wraps any `AiProvider` (complete / completeStructured / stream) and writes exactly one `AiRuns` row per call (status ok/error, tokens, latency) — **no chunk content, no prompt, no source text** is persisted, only metadata + the exception message. `makeProvider()` → `AnthropicProvider` iff `ANTHROPIC_API_KEY` else **`FakeProvider`** (deterministic, echoes source ids for exact citation asserts). `makeEmbedder()` → `VoyageEmbedder` iff `VOYAGE_API_KEY` else **`FakeEmbedder`** (sha256-per-token, 256-dim, L2-normalized). Offline-deterministic by default; all verification runs on the fakes.

### `ai.use` permission
Seeded (migration `0064`, mirroring the `0018_rbac.sql` CTE + `INSERT…WHERE NOT EXISTS` pattern, `Scope='WORKSPACE'` to satisfy `CK_Permissions_Scope`) to `workspace-owner`/`-admin`/`-member` only — **NOT viewer, NOT guest**. `POST /ai/search` gates on it via `requirePermission('ai.use', { resolveWorkspace from body })`; the search returns ONLY `retrieve()` output. (Because guests lack `ai.use` they get 403 at the HTTP layer, and any `ai.use` holder is a full member whose floor sees all scopes — so the per-user scope-filter is exercised at the service layer, not the route.)

### Indexing seams + scope anchoring
BullMQ `ai-index` queue/worker mirrors `recurrence.worker`; `runIndexJob(data)` is directly callable (tests drive it without Redis). Fire-and-forget, fail-open enqueue (`ai-index.service`, debounced 30s via `debounceGate`, guards falsy workspaceId/objectId). Seams added alongside existing side-effect calls in `task.service` (create/update/delete), `comment.service` (create/update/delete — delete resolves workspaceId BEFORE the row is gone), and **a NEW first side-effect seam in `docs.service`**. Scope anchors: **task → `LIST`/`ListId`**; **comment → its task's list**; **doc → `Docs.ScopeType/ScopeId`** (`ListId` only when `ScopeType='LIST'`). Worker bootstrap added to the `server.ts` `REDIS_URL||REDIS_HOST` block; the producer queue is registered for graceful close.

### Documented limitations / deferrals (carried + new)
- **Doc body lives in Yjs/collab persistence**, not `docs.service` — so the doc seam re-indexes on page-metadata ops (create/update/delete/restore) and picks up the latest persisted `DocPages.BodyJson` at worker-run time; **per-keystroke body indexing is NOT seamed** (would need a collab-persist hook — 11b+ follow-up).
- **`@anthropic-ai/sdk` 0.55.1** lacks `thinking:{type:'adaptive'}` and `messages.parse()` → `AnthropicProvider` uses `tool_choice:{type:'tool'}` structured output + `thinking:{type:'enabled'}` made **explicit opt-in** (`CompleteRequest.enableThinking`, default off, to avoid silently doubling cost when `maxTokens>1024`). The AnthropicProvider is **untested/opt-in (no LLM-answer surface in 11a)**; SDK upgrade + adaptive thinking + `parse()` deferred to 11b when the answer surface lands.
- Spec §8 deferrals retained: **ANN index, semantic similarity floor (results can pad to k with low-relevance chunks), reranking, real-provider replay tests**, the GraphQL mirror of `/ai/search` (REST-only in 11a; Pothos mirror lands in 11b with `aiAsk`).
- **AI integration tests must run serialized** (the integration vitest project sets `fileParallelism:false`; the whole AI dir is green with `--no-file-parallelism`). Running the dir in parallel surfaces shared-`ProjectFlow_Test`-DB contention false-reds, not code defects.
- **`POST /dev/ai/reindex`** (test/seed convenience): auth-gated + **404 in production** + **counts-only (never content)**, but no workspace-membership check — matches the existing `automation.dev.routes`/`scheduled-report.dev.routes` precedent. Not a data-exposure vector.
