...
export default function ProjectsPage() {
  ...
  // ── Projects ───────────────────────────────────────────────────────────────
  const { data: projects, isLoading: isLoadingProj } = useQuery<ApiProject[]>({
    queryKey: ['projects-manage', activeWorkspaceId, accessToken],
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ['projects-manage', activeWorkspaceId] });

  const createMutation = useMutation({
    mutationFn: async (input: {
      name: string; key: string; type: ProjectType; description: string;
    }) => {
      const { ok, json } = await api('/projects', accessToken, {
        method: 'POST',
        body: JSON.stringify({
    ...
    onSuccess: () => {
      invalidate();
      // Also refresh the global cached project lists used by switchers
  ...
  const archiveMutation = useMutation({
    mutationFn: async (id: string) => {
      const { ok, json } = await api(`/projects/${id}/archive`, accessToken, { method: 'POST' });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { ok } = await api(`/projects/${id}`, accessToken, { method: 'DELETE' });
  ...
  const filtered = useMemo(() => {
    ...
    return (projects ?? []).filter((p) => {
      const pType   = String(p.Type   ?? p.type   ?? 'KANBAN');
      const pStatus = String(p.Status ?? p.status ?? 'ACTIVE');
      if (typeFilter   !== 'ALL' && pType   !== typeFilter)   return false;
      if (statusFilter !== 'ALL' && pStatus !== statusFilter) return false;
      if (q) {
        const name = String(p.Name ?? p.name ?? '').toLowerCase();
        const key  = String(p.Key  ?? p.key  ?? '').toLowerCase();
        const desc = String(p.Description ?? p.description ?? '').toLowerCase();
        if (!name.includes(q) && !key.includes(q) && !desc.includes(q)) return false;
      }
      return true;
    });
  ...
  const kpi = useMemo(() => {
    ...
    return {
      total:    all.length,
      active:   all.filter((p) => String(p.Status ?? p.status) === 'ACTIVE').length,
      archived: all.filter((p) => String(p.Status ?? p.status) === 'ARCHIVED').length,
      kanban:   all.filter((p) => String(p.Type ?? p.type) === 'KANBAN').length,
    };
  }, [projects]);

  // ── Derived UI state ───────────────────────────────────────────────────────
  const isInitialLoading = isLoadingWs || (!!activeWorkspaceId && isLoadingProj && !projects);
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-muted/30 p-2">
            ...
            <div className="ml-auto text-xs text-muted-foreground">
              Showing <strong className="text-foreground">{filtered.length}</strong> of <strong className="text-foreground">{projects?.length ?? 0}</strong>
            </div>
          </div>
              ...
              {filtered.map((p) => (
                <ProjectCard
                  ...
                  onArchive={() => {
                    ...
                    if (window.confirm(`Archive ${name}?\n\nArchived projects stay readable but won't appear in switchers by default.`)) {
                  ...
                  onDelete={() => {
                    const id = String(p.Id ?? p.id);
                    const name = p.Name ?? p.name;
                    if (window.confirm(`Delete ${name}?\n\nThis soft-deletes the project. All its issues, sprints, and workflow stay in the database but become invisible.`)) {
...
function ProjectCard({
  ...
  const name   = project.Name ?? project.name ?? '(unnamed)';
  const key    = project.Key  ?? project.key  ?? '—';
  const desc   = project.Description ?? project.description;
  const type   = String(project.Type   ?? project.type   ?? 'KANBAN') as ProjectType;
  const status = String(project.Status ?? project.status ?? 'ACTIVE') as ProjectStatus;
  const created = project.CreatedAt ?? project.createdAt;
    ...
    <Card className={cn('p-4 flex flex-col gap-3', status !== 'ACTIVE' && 'opacity-70')}>
      ...
      {created && (
        <span className="text-[10px] uppercase tracking-wide text-muted-foreground/70 font-mono">
          Created {new Date(created).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
        </span>
      )}
    </Card>
  );
