...
// ── Paths ────────────────────────────────────────────────────────────────────
const MIGRATIONS_DIR = path_1.default.resolve(path_1.default.dirname(decodeURIComponent(new URL(import.meta.url).pathname.replace(/^\/([A-Z]:)/, '$1'))), '../infra/sql/migrations');
// ── Helpers ──────────────────────────────────────────────────────────────────
...
async function getApplied(pool) {
    const result = await pool.request().query('SELECT FileName FROM dbo.MigrationHistory ORDER BY Id');
    return new Set(result.recordset.map((r) => r.FileName));
}
async function recordMigration(pool, fileName, checksum) {
    await pool.request()
        .input('FileName', mssql_1.default.NVarChar(255), fileName)
        .input('Checksum', mssql_1.default.NVarChar(64), checksum)
...
async function main() {
    ...
    const entries = await fs_1.promises.readdir(MIGRATIONS_DIR);
    ...
    for (const file of pending) {
        ...
        try {
            ...
            if (e.number)
                console.error(`  SQL error number: ${e.number}`);
            if (e.lineNumber)
                console.error(`  Batch line:       ${e.lineNumber}`);
            if (e.procName)
                console.error(`  Procedure:        ${e.procName}`);
            process.exitCode = 1;
            break;
        }
    }
