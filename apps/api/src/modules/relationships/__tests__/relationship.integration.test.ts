...
describe('Phase 5b — relationship + rollup (integration)', () => {
  it('links a task across two lists and lists the ToTask ref', async () => {
    ...
    const refs = await relationshipService.list(relField.id, a);
    ...
    // remove clears it
    const removed = await relationshipService.remove(relField.id, a, b);
    expect(removed).toBe(1);
    expect(await relationshipService.list(relField.id, a)).toHaveLength(0);
  });
