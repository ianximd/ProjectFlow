    ...
    <Popover onOpenChange={ensureMembers}>
      ...
      <PopoverContent align="start" className="w-64 p-2">
        <div className="flex flex-col gap-1">
          {loading && <span className="px-2 py-1 text-xs text-muted-foreground">Loading members…</span>}
          {members && members.length === 0 && (
            <span className="px-2 py-1 text-xs text-muted-foreground">No members</span>
          )}
          {members?.map((m) => (
